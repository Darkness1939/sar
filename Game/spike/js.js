...
{ 
    ...
    if(health <= 0)
 {
    ...
    document.getElementById('text').innerHTML = `You dead! <button id="dres" onclick="woo();">Reset</button>`;
    ...
    if((x>=1620&&x<=1720)&&(y>=250&&y<=550)){
        document.getElementById('help').innerHTML = `<b>Are you sure you want to come here?</b><button onclick="document.location='D:/game/Game/egypt/index.html'" id="yeah">Yes</button> <button onclick="no2()" id="nah">No</button>`
        document.getElementById('help').style.display = 'flex';
    }
    }    
}
    ...
    function fight(){
        const block = document.getElementById("enemy");
        ...
        var timer1 = setInterval("if(health > 0) {health = health - 2; document.getElementById('G').innerText = `${health}`; endh = false;} else{endh = true;  document.getElementById('text').style.display= 'flex'; document.getElementById('text').innerHTML = `TOTEM! <button id= 'dres' onclick= 'woo()';>Reset</button>`; health = 0; document.getElementById('G').innerText = 0;}", 750);
        var timer2 = setInterval("if(energy<1000){ energy = energy + 7; document.getElementById('mana').innerText = `${energy}`;} if(energy>1000){energy = 1000}; document.getElementById('mana').innerText = `${energy}`;", 500);
        window.onkeyup = function punch(){
    if(!endh){
    if(!endo){
        if(event.keyCode==32&& document.getElementById('enemy').style.backgroundColor == 'white'){
           ehealth = ehealth - 25;
           document.getElementById('enemyheal').innerHTML = `${ehealth}`;
               }
        if(event.keyCode==32 && document.getElementById('enemy').style.backgroundColor == 'black'){
            ehealth = ehealth - 25;
            health = health - 5;
            document.getElementById('enemyheal').innerHTML = `${ehealth}`;
            document.getElementById('G').innerHTML = `${health}`;
        }
        }
        if(!andf){
            if((event.keyCode==81&&energy>=100)&&document.getElementById('enemy').style.backgroundColor == 'blue'){
                ehealth = ehealth - 200;
                energy = energy - 200;
